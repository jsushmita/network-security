...
private String MONITOR_PASSWORD;
   ...
   public boolean Login() { 
      ...
      try {  
    	  String monitorMsg = GetMonitorMessage();
    	  
    	  if(monitorMsg.replaceAll("\\s+","").contains("REQUIRE:IDENT")){
    		  Execute("IDENT");
    		  monitorMsg = GetMonitorMessage();
    	  }
    	  if(monitorMsg.replaceAll("\\s+","").contains("REQUIRE:PASSWORD")){
    		  Execute("PASSWORD");
    		  monitorMsg = GetMonitorMessage();
    		  System.out.println(monitorMsg);
    		  COOKIE = monitorMsg.substring(monitorMsg.indexOf("PASSWORD")+9, monitorMsg.indexOf("REQUIRE")-1); 
    	  }
    	  if(monitorMsg.replaceAll("\\s+","").contains("REQUIRE:HOST_PORT")){
    		  Execute("HOST_PORT");
    		  monitorMsg = GetMonitorMessage();
    	  }
    	  if(monitorMsg.replaceAll("\\s+","").contains("REQUIRE:ALIVE")){
    		  Execute("ALIVE");
    		  monitorMsg = GetMonitorMessage();
    	  }
    	  if(monitorMsg.replaceAll("\\s+","").contains("REQUIRE:QUIT"))
    		  Execute("QUIT");
    	  success = true;
      } catch (NullPointerException n) {
   ...
   public boolean Execute (String sentmessage) {
      ...
      try {
         if (sentmessage.trim().equals("IDENT")) {
            ...
            sentmessage = sentmessage.concat(" ");
            sentmessage = sentmessage.concat(SERVER_HOSTNAME);//hostname
            sentmessage = sentmessage.concat(" ");
